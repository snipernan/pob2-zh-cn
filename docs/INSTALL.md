...
适用于 **已有 Apple Silicon Mac 原生 PoB2** 的用户，已验证核心 **0.23.1**。安装脚本使用 **Python 3.9+**；插件在 PoB2 内通过 Lua 运行。
...
核心更新可能覆盖插件加载入口。更新后检查状态，按需重新安装并重启 PoB2，即可恢复汉化。

默认目录卸载命令为 `python3 install.py --uninstall`，也可双击 `卸载汉化.command`。卸载移除插件入口并保留用户配装，重新打开原来的 PoB2 后生效。

## 启用中文显示

独立插件用户可运行 `python3 install.py --status`。如入口缺失，重新安装并重启。核心更新可能重新写入 `Launch.lua`；上游更新按原有流程运行。磁盘状态正常但窗口仍是英文时，确认重启的是相应 App，按 F10 或 Fn + F10 切换语言。
