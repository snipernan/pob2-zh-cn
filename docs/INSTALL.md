...
适用于 **已有 Apple Silicon Mac 原生 PoB2** 的用户，已验证核心 **0.23.1**。当前独立插件版本为 **0.6.4**。安装脚本使用 **Python 3.9+**；插件在 PoB2 内通过 Lua 运行。
...
v0.6.4 兼容编年史 WeGame 转换结果中 `Fire Resistance is +22%` 这类带显式加号的火焰、冰霜、闪电与混沌抗性，按原值交给核心加法抗性解析。兼容层保留装备原文和导出内容，切换到英文界面也持续生效。安装后重启 PoB2，并重新打开已保存配装即可重新计算。

字库包含 GB2312 的 6763 个常用汉字与已收录译文用字。名称中的 `[U+5E86]` 是原生绘制器缺少“庆”的显示提示；原文件名仍保留中文。升级并重启后可正常显示。更罕见的汉字可按开发指南继续扩充图集。
